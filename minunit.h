...
#define mu_assert_str_equal(actual, expected)                                  \
  mu_assert(strcmp(actual, expected) == 0, "'%s' does not equal '%s'", actual, \
            expected);
...
#define mu_assert_int_equal(actual, expected)                                  \
  mu_assert(expected == actual, "%s (%i) does not equal %i", #actual, actual,  \
            expected);
...
#define test_results(test_run)                                                 \
  printf("%d tests run, %d assertions, %d failed\n", test_run->tests_run,      \
         test_run->asserts_run, test_run->asserts_failed);
...
#define RUN_TESTS(name)                                                        \
  int main(int argc, char *argv[]) {                                           \
    ...
    exit(test_run->asserts_failed != 0);                                       \
  }
